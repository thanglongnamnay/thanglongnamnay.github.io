...
const checker = meaningfulWords => {
	...
	const check = async uglyWord => {
		...
		const word = removeMark(uglyWord).toLowerCase();
...
const markWordAsWrong = wrongWords => {
	const words = textInput.textContent.split(' ');
	textInput.innerHTML = words.map(w => wrongWords.includes(removeMark(w))
...
const handleCheck = checker => async e => {
	...
	const words = seperate(textInput.textContent).split(' ').map(removeMark).filter(notEmpty);
	console.log(words);
