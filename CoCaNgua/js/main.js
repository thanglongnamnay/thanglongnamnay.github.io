...
let thinkTime = 250, fastMode = false;
const $ = (selector) => document.querySelectorAll(selector); //jQuery LUL
const trunc = (x) => (x + 56) % 56;
const randomDiceRoll = () => Math.floor(Math.random() * 6) + 1;
const log = (name) => console.log(`Something went wrong with '${name}'`);
class Box {
    constructor(_position, _team = -1) {
    ...
    addSclick(func) {
        ...
        this.DOM.addEventListener('sclick', this.sclickFunc[this.sclickFunc.length - 1]);
    }
    removeSclick() {
        if (this.sclickFunc)
    ...
    changeLight(mode) {
        if (mode === 0) {
            this.DOM.style.filter = 'brightness(100%)';
            return true;
        }
        if (mode === 1) {
            this.DOM.style.filter = 'brightness(85%)';
            return true;
        }
        if (mode === 2) {
            this.DOM.style.filter = 'brightness(70%)';
            return true;
        }
        log('changeLight');
        return false;
...
let boxes = [];
let pawnPos = [[-7, -7, -7, -7], [-7, -7, -7, -7], [-7, -7, -7, -7], [-7, -7, -7, -7]];
let dices = [];
let currentTeam = 0, players = 1;
function play(team, turn) {
    if (!fullCheck(team)) {
        nextTurn();
        return false;
    }
...
function think(team, dices, positions) {
    ...
    function chooseBestNumber(arr) {
        let newarr = arr.slice(0), sum = 0, sumArr = [];
        ...
        let r = Math.round(Math.random() * sum), i = 0;
        while (r > sumArr[i])
            i++;
        return arr.indexOf(newarr[i]);
    }
    function benchmark(team, wayToGo) {
...
function diceOnSclick(on) {
    getBox(-8).removeSclick();
    if (on)
        getBox(-8).addSclick(rollDice);
    else
        getBox(-8).addSclick(nextTurn);
}
function win(team) {
    for (let pos of pawnPos[team])
...
function emphasize(team, pos, dices) {
    let dice = dices[dices.length - 1];
    let checkValue = [check(team, pos, dice)];
    let goodToGo = [];
    if (dice !== dices[0])
...
function diceSound() {
    const audioIndex = Math.floor((Math.random()*6)+1);
    return new Audio('/CoCaNgua/sound/xx'+audioIndex+'.wav');
}
function rollDice() {
    diceOnSclick(false);
...
function showDices(team, dices) {
    let kq = $('.ketqua')[0];
    kq.innerHTML = '';
    for (let dice of dices)
        kq.innerHTML += dice + ' ';
}
function getBox(position, team) {
    if (position === -8) {
        return boxes[boxes.length - 1];
    }
    else if (position < 0) {
        for (let box of boxes) {
            if (box.position === position && box.team === team)
                return box;
        }
    }
    else {
...
function check(team, pos, dice) {
    let positions = getPositions();
    if (pos === -7) {
        if (dice !== 6)
            return null;
        if ((positions[team]).indexOf(keyPosition.first[team]) >= 0)
            return null;
        return keyPosition.first[team];
    }
    if (pos < 0) {
        if (-dice !== pos - 1 || exist(pos - 1, team))
            return null;
        return -dice;
    }
    if (pos === keyPosition.last[team]) {
        for (let i = 1; i <= dice; ++i)
            if (positions[team].indexOf(-i) >= 0)
                return null;
        return -dice;
    }
    if (pos >= 0 && pos < 56) {
        ...
        if (nearest.distant < dice)
            return null;
        if (nearest.distant === dice &&
            (positions[team].indexOf(trunc(pos + dice)) >= 0))
            return null;
        return trunc(pos + dice);
    }
    log('check');
    return null;
    function getNearestPositionAndDistant(team, position, positions) {
...
function setup() {
    ...
    diceOnSclick(true);
    $('.nowhere')[0].onclick = () => location.reload();
    document.onkeyup = (e) => {
        if (e.key == 'Enter')
            $('.dice')[0].click();
...
function moveDice(team) {
    ...
    let ketQua = $('.ketqua')[0], control = $('.controls')[0];
    switch (team) {
        case 0:
            control.style.top = '3%';
...
function drawBoard(playersNumber = 0) {
    fastMode = $('#fast-mode')[0].checked;
